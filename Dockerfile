...
COPY package.json yarn.lock* package-lock.json* pnpm-lock.yaml* .yarnrc.docker-simple.yml ./
# Use Docker-specific Yarn config for node-modules linker
...
WORKDIR /app
# Copy node_modules and basic configuration
COPY --from=deps /app/node_modules ./node_modules
COPY --from=deps /app/package.json ./package.json
COPY --from=deps /app/.yarnrc.yml ./.yarnrc.yml

# Copy source files but exclude PnP files
...
WORKDIR /app
# Copy node_modules and basic configuration for builder stage
COPY --from=deps /app/node_modules ./node_modules
COPY --from=deps /app/package.json ./package.json
COPY --from=deps /app/.yarnrc.yml ./.yarnrc.yml
COPY . .
# Ensure Drizzle config is present
COPY drizzle.config.ts ./
...
CMD ["node", "/app/.yarn/releases/yarn-4.9.2.cjs", "--", "node", "/app/server.js"]
